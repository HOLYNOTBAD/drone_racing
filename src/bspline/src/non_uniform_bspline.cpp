...
Eigen::VectorXd NonUniformBspline::evaluateDeBoor(const double& u) {
  if (control_points_.rows() == 0) return Eigen::VectorXd::Zero(3);
  ...
  for (int i = 0; i <= p_; ++i) {
    int idx = k - p_ + i;
    if (idx < 0) idx = 0;
    if (idx >= control_points_.rows()) idx = control_points_.rows() - 1;
    d.push_back(control_points_.row(idx));
...
Eigen::VectorXd NonUniformBspline::evaluateDeBoorT(const double& t) {
  if (u_.size() <= p_ || p_ < 0) return Eigen::VectorXd::Zero(3);
