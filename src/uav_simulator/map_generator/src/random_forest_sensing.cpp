...
// default_random_engine eng(42); //随机数引擎
mt19937 eng(43);  //随机数引擎
...
// 地图文件路径（可通过参数配置）
string _map_file_path = "/home/rfly603/IL_planner_ego/src/uav_simulator/map_generator/random_map.pcd";

void RandomMapGenerate();
void FixedMapGenerate();

/*
    将点云地图保存到文件（二进制格式，更快更小）
*/
bool SaveMapToFile(const string& file_path) {
  ...
  // 写入点的数量
  size_t num_points = cloudMap.points.size();
  ...
  ofs.close();
  ROS_WARN("Map saved to %s with %lu points", file_path.c_str(), num_points);
  return true;
}
...
/*
    从文件加载点云地图
*/
bool LoadMapFromFile(const string& file_path) {
  ifstream ifs(file_path, ios::binary);
  if (!ifs.is_open()) {
    ROS_ERROR("Failed to open file for reading: %s", file_path.c_str());
    return false;
  }
  ...
  // 读取所有点
  cloudMap.points.reserve(num_points);
  ...
  ROS_WARN("Map loaded from %s with %lu points", file_path.c_str(), num_points);
  return true;
}
...
*/
void FixedMapGenerate() {
  if (!LoadMapFromFile(_map_file_path)) {
    ROS_ERROR("Failed to load map, falling back to RandomMapGenerate");
    RandomMapGenerate();
    // 保存生成的地图供下次使用
    SaveMapToFile(_map_file_path);
  }
}
...
void rcvOdometryCallbck(const nav_msgs::Odometry odom) {
  ...
  _has_odom = true;
  ROS_INFO_ONCE("[Map server] received odometry info.");
...
void pubSensedPoints() {
  ...
  ROS_INFO_ONCE("[Map server] publish local map.");
...
int main(int argc, char** argv) {
  ...
  // 订阅洪立写的fake_odom节点发布的里程计信息，这样random_map_sensing (which uses a private NodeHandle) can subscribe.
  //_odom_sub = n.subscribe("/fake_odom/odometry", 50, rcvOdometryCallbck);

  click_map_pub_ =
      n.advertise<sensor_msgs::PointCloud2>("/pcl_render_node/local_map", 1);

  /*这行可以订阅rviz的2D Nav Goal消息 并且在对应的位置生成一个随机大小的长方体障碍物*/
  ...
  // 地图文件相关参数
  n.param<string>("map/file_path", _map_file_path, "/home/rfly603/IL_planner_ego/src/uav_simulator/map_generator/random_map.pcd");
  bool use_fixed_map = false;
  n.param("map/use_fixed_map", use_fixed_map, false);
  ...
  if (use_fixed_map) {
    FixedMapGenerate();  // 从文件加载地图
  } else {
    RandomMapGenerate();  // 随机生成并保存地图
  }
